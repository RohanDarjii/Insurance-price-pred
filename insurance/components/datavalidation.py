...
import numpy as np
from insurance.config import TARGET_COL
...
class DataValidation:
    ...
    def drop_missing_values_columns(self, df:pd.DataFrame, report_key_name:str)->Optional[pd.DataFrame]:
        try:
            ...
            logging.info(f"selecting column name which contains null value above threshold {threshold}")
            drop_col_name = null_report[null_report>threshold].index
            
            logging.info(f"Dropping column {list(drop_col_name)}")
            self.validation_error[report_key_name] = list(drop_col_name)
            df.drop(list(drop_col_name), axis=1, inplace=True)
    ...
    def is_required_columns_exist(self, base_df:pd.DataFrame, current_df:pd.DataFrame, report_key_name:str)->bool:
        try:  #base data == Original data
            ...
            #Creating report if missing columns is present
